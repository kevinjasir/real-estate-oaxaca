...
async function getProject(slug: string): Promise<ProjectData | null> {
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/projects?slug=eq.${encodeURIComponent(slug)}&status=eq.active&limit=1`,
    ...
    const [mediaRes, lotsRes] = await Promise.all([
      fetch(
        `${SB_URL}/rest/v1/media?entity_type=eq.project&entity_id=eq.${project.id}&select=url,order_index&order=order_index.asc`,
        { headers: SB_HEADERS, cache: "no-store" }
      ),
      fetch(
        `${SB_URL}/rest/v1/lots?project_id=eq.${project.id}&select=id,lot_number,status,price,size_m2&order=lot_number.asc`,
        { headers: SB_HEADERS, cache: "no-store" }
      ),
    ]);

    const media: { url: string; order_index: number }[] = mediaRes.ok
      ? await mediaRes.json()
      : [];
    const lots: LotData[] = lotsRes.ok ? await lotsRes.json() : [];
    const gallery = media.map((m) => m.url);

    return {
      ...project,
      hero_image: project.cover_image_url || gallery[0] || null,
      gallery,
      lots,
    };
  } catch (error) {
...
export default async function ProjectDetailPage({ params, searchParams }: PageParams) {
  ...
  const availableLots = project.lots?.filter((l) => l.status === "available") || [];
  const whatsappNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "529511234567";
          ...
          <div className="container mx-auto">
            ...
            {project.location_name && (
              <p className="text-xl text-gray-200 flex items-center gap-2">
                ...
                {project.location_name}
                {project.city && project.city !== project.location_name && `, ${project.city}`}
              </p>
            )}
          </div>
        </div>
      ...
      <section className="bg-white border-b shadow-sm py-4">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 text-center">
            <div>
              <p className="text-2xl font-bold text-emerald-600">{formatPrice(project.price_from)}</p>
              <p className="text-sm text-gray-500">Precio desde</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-gray-900">{availableLots.length}</p>
              <p className="text-sm text-gray-500">Lotes disponibles</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-gray-900">{formatArea(project.lot_size_from)}</p>
            ...
            <div className="lg:col-span-2 space-y-8">
              {project.description && (
                <div className="bg-white rounded-2xl p-6 shadow-lg">
                  <h2 className="text-2xl font-bold text-gray-900 mb-4">Acerca del proyecto</h2>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-8 space-y-6">
                <div className="bg-white rounded-2xl p-6 shadow-lg border-2 border-emerald-100">
                  <h3 className="text-xl font-bold text-gray-900 mb-2">¿Te interesa este proyecto?</h3>
                ...
                <div className="bg-emerald-50 rounded-2xl p-6">
                  <h4 className="font-semibold text-emerald-900 mb-4">Resumen del proyecto</h4>
                  <ul className="space-y-3 text-sm">
                    <li className="flex justify-between">
                      <span className="text-gray-600">Total de lotes:</span>
                      <span className="font-medium text-gray-900">{project.total_lots || 0}</span>
                    </li>
                    <li className="flex justify-between">
                      <span className="text-gray-600">Disponibles:</span>
                      <span className="font-medium text-emerald-600">{availableLots.length || project.available_lots || 0}</span>
                    </li>
                    <li className="flex justify-between">
                      <span className="text-gray-600">Precio desde:</span>
                      <span className="font-medium text-gray-900">{formatPrice(project.price_from)}</span>
                    </li>
                    <li className="flex justify-between">
                      <span className="text-gray-600">Tamaño desde:</span>
                      <span className="font-medium text-gray-900">{formatArea(project.lot_size_from)}</span>
                    </li>
                  </ul>
                </div>
              </div>
